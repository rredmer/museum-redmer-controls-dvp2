 ...
 *
 *	Notes:	All densities are expressed in integer form, scaled by 1000, for example:
 *
...
#define	GB_TARGET_MOD_METHOD_LINEAR		0	/* linearly transform targets to actual media range */
#define	GB_TARGET_MOD_METHOD_CLAMP		1	/* match actual targets, if possible, else clamp */
#define	GB_TARGET_MOD_METHOD_LINEAR_MINMAX	2	/* linearly transform targets to media min/max range */

/* - - - - -  typedefs  - - - - - */

typedef	int		GbColorInt[3];	/* used for density, pixel value or exposure set, RGB order */
...
GbExamineDensities(
	int		nMeas,		/* number of measurements in array measDens */
					/* (must be one of 8, 13, 16, 24, 64, 72, 208, 344, 1024) */
...
GbRefineExposureSet(			/* refine current exposure set */
	...
	int		nMeas,		/* number of measurements in measDens (13 or 30 only) */
	GbColorInt	*measDens	/* pointer to array of nMeas density measurements */
);
...
GbRefineGrayMap(			/* refine current gray balance map */
	...
	GbMap		*outGMap,	/* pointer to output gray balance map (may be same as inGMap) */
	int		nMeas,		/* number of measurements in measDens (4 * steps in test pattern) */
